...
Module Module1
    ...
    Private msg, thisprocess, xmlProcess, message, stack As String
    ...
    Sub Main()
        ...
        server = txtArray(1)
        dbase = txtArray(2)
        rcExePath = txtArray(3)
        xmlPath = txtArray(4)
        sqlUserId = txtArray(5)
        sqlPassword = txtArray(6)
        thisSdate = txtArray(7)
        thisEdate = txtArray(8)
        rcErrorPath = txtArray(9)
        cust = txtArray(10)
        xmlProcess = txtArray(11)

        ''Console.WriteLine("client=" & client)
        ''Console.WriteLine("server=" & server)
        ''Console.WriteLine("dbase" & dbase)
        ''Console.WriteLine("exepath=" & rcExePath)
        ''Console.WriteLine("xmlpath=" & xmlPath)
        ''Console.WriteLine("sqluserid=" & sqlUserId)
        ''Console.WriteLine("sqlpassword=" & sqlPassword)
        ''Console.WriteLine("thissdate=" & thisSdate)
        ''Console.WriteLine("thisedate=" & thisEdate)
        ''Console.WriteLine("errorpath=" & rcErrorPath)
        ''Console.WriteLine("cust=" & cust)
        ''Console.WriteLine("xmlprocess=" & xmlProcess)
        ''Console.ReadLine()
        ...
        'rcExePath = "c:\RetailClarity\EXEs\v1.3"
        'cust = "N"
...
100:        Try                 '    Create records for the next week when processing the last day of the week
            ...
            Catch ex As Exception
                ...
                message = ex.Message
                stack = ex.StackTrace
                Threading.Thread.Sleep(1000)                ' wait for 1 second and try again
            End Try
        Loop Until ok OrElse retries >= 3
        If Not ok Then
            If con.State = ConnectionState.Open Then con.Close()
            If con2.State = ConnectionState.Open Then con2.Close()
            Dim el As New XMLUpdate.ErrorLogger
            el.WriteToErrorLog(message, stack, thisprocess)
        End If
        ...
        Do
            Try
                ...
                Dim item, descr, vid, vendor, vitem, note, buyer, dept, clss, custom1, custom2, custom3, custom4, custom5, uom,
                    sku, dim1, dim2, dim3, dim1_descr, dim2_descr, dim3_descr, status, sql, type, lastUser As String
                ...
                dt2.Columns.Add("Last_UpDate", GetType(System.DateTime))
                dt2.Columns.Add("Last_User")
                Dim xmlFile As XmlReader
                Dim rw As DataRow
                ...
                If ((ds.Tables.Count > 0) AndAlso ds.Tables(0).Rows.Count > 0) Then
                    ...
                    For Each row In dt.Rows
                        ...
                        If sku <> "TOTALS" Then
                            ...
                            oTest = row("LAST_USER")
                            If IsDBNull(oTest) Then lastUser = Nothing Else lastUser = CStr(oTest)
                            ...
                            rw("LAST_UPDATE") = lastUpdate
                            rw("LAST_USER") = lastUser
                            dt2.Rows.Add(rw)
                        End If
                    Next
                End If
    ...
    Private Sub Process_Barcodes(ByVal thePath, con, con2, constr)
        ...
        Do
            ...
            Catch ex As Exception
                ...
                Threading.Thread.Sleep(1000)                ' wait for 1 second and try again
            End Try
        Loop Until ok OrElse retries >= 0
        If Not ok Then
            If con.State = ConnectionState.Open Then con.Close()
            Dim el As New XMLUpdate.ErrorLogger
            el.WriteToErrorLog(message, stack, thisprocess)
        ...
        Do
            Try
                ...
                sql = "CREATE TABLE _t1(" &
                       ...
                       "[LAST_CHANGE_USER] [varchar](40) NULL) ON [PRIMARY]"
                cmd = New SqlCommand(sql, con2)
                cmd.CommandTimeout = 480
                cmd.ExecuteNonQuery()
                ...
                Dim item, dim1, dim2, dim3, sku, loc, lastUser As String
                Dim cost, retail, tcost, tretail As Decimal
                Dim dte As Date = Date.Today
                Dim dttime, lastUpdate As DateTime
                Dim ds As DataSet = New DataSet
                Dim dt As DataTable = New DataTable
                Dim rdr As SqlDataReader
                    ...
                    For Each row In dt.Rows
                        If row("Sku") <> "TOTALS" Then
                            ...
                            sql = "INSERT INTO _t1 (Loc_Id, Sku, sDate, eDate, End_OH, " &
                                     "Cost, Retail, YrWk, Item, DIM1, DIM2, DIM3, LAST_CHANGE_DATE, LAST_CHANGE_USER) " &
                                     "SELECT '" & loc & "','" & sku & "','" & thisSdate & "','" & thisEdate &
                                     "'," & onhand & "," & cost & "," & retail & ", YrWk ,'" & item & "','" & dim1 & "','" & dim2 & "','" &
                                     dim3 & "','" & lastUpdate & "','" & lastUser & "' FROM Calendar c " &
                                     "JOIN Item_Master m ON m.Sku = '" & sku & "' " &
                                     "WHERE eDate = '" & thisEdate & "' AND m.[Type] = 'I' AND Prd_Id > 0 AND Week_Id > 0"
                            cmd = New SqlCommand(sql, con2)
                ...
                sql = "MERGE Item_Inv AS target USING _t1 AS source ON (source.Loc_Id = target.Loc_Id AND source.Sku = target.Sku " &
                          ...
                          "VALUES (source.Loc_Id, source.Sku, source.sDate, source.eDate, source.Cost, source.Retail, " &
                              ...
                              "source.Item, source.DIM1, source.DIM2, source.DIM3, source.LAST_Change_Date, source.Last_Change_User) " &
                          "WHEN MATCHED THEN " &
                          ...
                          "target.Last_Change_Date = source.Last_Change_Date, target.Last_Change_User = source.Last_Change_User;"
                cmd = New SqlCommand(sql, con)
                cmd.CommandTimeout = 960
                cmd.ExecuteNonQuery()
                    ...
                    For Each row In dt.Rows
                        ...
                        If oTest <> "TOTALS" Then
                            ...
                            sql = "INSERT INTO Purchase_Request_Header(PREQ_NO, Batch_Id, Loc_Id, " &
                                ...
                                alloc & "','" & mrg & "','" & lastUpdate & "','" & terms & "','" & custom & "','" &
                    ...
                    For Each row As DataRow In dt.Rows
                        ...
                        If oTest <> "TOTALS" Then
                            ...
                            sql = "INSERT INTO Purchase_Request_Detail(PREQ_NO, Seq_No, Sku, Description, " &
                                "Stock_Units, Numer, Denom, Cost, Item, DIM1, DIM2, DIM3, Qty, LAST_UPDATE) " &
                                "SELECT '" & preq & "'," & seq & ",'" & sku & "','" & desc & "','" &
                                uom & "'," & numer & "," & denom & "," & cost & ",'" & item & "','" &
                                dim1 & "','" & dim2 & "','" & dim3 & "'," & qty & ",'" & lastUpdate & "'"
                            cmd = New SqlCommand(sql, con2)
                            cmd.CommandTimeout = 480
                            cmd.ExecuteNonQuery()
                        Else
    ...
    Private Sub Process_POHeader(ByVal thePath, ByVal con, ByVal con2)
        ...
        Do
            Try
                ...
                Dim datetime, lastUpdate As DateTime
                Dim ds As DataSet = New DataSet
                Dim dt As DataTable = New DataTable
                    ...
                    For Each row In dt.Rows
                        ...
                        If oTest <> "TOTALS" Then
                            ...
                            sql = "IF NOT EXISTS (SELECT PO_NO FROM PO_Header WHERE PO_NO = '" & po & "') " &
                                ...
                                "Amount, Recvd_Cost, Lines, Ord_Qty, Open_Lines, Open_Amt, Terms, Custom1, Custom2, Custom3, Custom4, " &
                                "Custom5, LAST_UPDATE) " &
                                ...
                                custom3 & "','" & custom4 & "','" & custom5 & "','" & lastUpdate & "'"
                            cmd = New SqlCommand(sql, con)
                            cmd.CommandTimeout = 480
                            cmd.ExecuteNonQuery()
                        Else
    ...
    Private Sub Process_PODetail(ByVal thepath, ByVal con, ByVal con2)
        ...
        Do
            Try
                ...
                Dim extractDate, lastUpdate As DateTime
                Dim cmd As SqlCommand
                Dim ds As DataSet = New DataSet
                Dim dt As DataTable = New DataTable
                        ...
                        If oTest <> "TOTALS" Then
                            ...
                            If lastDate <> "" Then
                                ...
                                sql = "IF NOT EXISTS (SELECT PO_NO FROM PO_Detail WHERE PO_NO = '" & po & "' " &
                                    ...
                                    "Cost, Retail, Last_Recvd_Date, Item, DIM1, DIM2, DIM3, LAST_UPDATE) " &
                                    "SELECT '" & loc & "','" & po & "'," & seq & ",'" & item & "'," & oqty & "," & rqty & "," &
                                    eqty & "," & cost & "," & retail & ",'" & lastDate & "',Item, DIM1, DIM2, DIM3, '" &
                                    lastUpdate & "' FROM Item_Master " &
                                    "WHERE Sku = '" & item & "'"
                            Else
                                sql = " IF NOT EXISTS (SELECT PO_NO FROM PO_Detail WHERE PO_NO = '" & po & "' " &
                                    "AND Sku = '" & item & "' AND Seq_No = " & seq & ") " &
                                    ...
                                    "Cost, Retail, Last_Recvd_Date, Item, DIM1, DIM2, DIM3, LAST_UPDATE) " &
                                    "SELECT '" & loc & "','" & po & "'," & seq & ",'" & item & "'," & oqty & "," & rqty & "," &
                                    eqty & "," & cost & "," & retail & ", NULL, Item, DIM1, DIM2, DIM3, " & lastUpdate & "' FROM Item_Master " &
                                    "WHERE Sku = '" & item & "'"
                            End If
                            cmd = New SqlCommand(sql, con)
                            cmd.CommandTimeout = 480
    ...
    Private Sub Process_Data(ByVal thePath, ByVal theField, con, con2)
        ...
        Do
            Try
                ...
                Dim custom, custom2, custom3, custom4, custom5, lastUser As String
                Dim seq As Int32
                ...
                If ((ds.Tables.Count > 0) AndAlso ds.Tables(0).Rows.Count > 0) Then
                    ...
                    For Each row In dt.Rows
                        ...
                        If Not IsDBNull(oTest) AndAlso oTest <> "TOTALS" Then
                            ...
                            Dim checkDate As DateTime = transDate
                            If theField <> "Sold" And theField <> "Order" Then checkDate = lastUpdate
                            Dim itexists As Boolean = Check_Log(con2, id, seq, loc, sku, checkDate, store, theField)
                            ...
                            If Not itexists Then
                                ...
                                sql = "SELECT eDate FROM Calendar WHERE '" & CDate(checkDate) & "' BETWEEN sDate AND eDate AND Week_ID > 0"
                                cmd = New SqlCommand(sql, con2)
                                cmd.CommandTimeout = 120
                                rdr = cmd.ExecuteReader
                                While rdr.Read
                                ...
                                If qty <> 0 Then                            ' Don't do anything unless qty <> 0
                                    ...
                                    Else
                                        ...
                                        "RETAIL, EXTRACT_DATE, POST_DATE, ITEM, DIM1, DIM2, DIM3, LAST_CHANGE_DATE, LAST_CHANGE_USER) " &
                                        "SELECT '" & id & "', " & seq & ", '" & sku & "', '" & loc & "', '" & transDate & "', '" & transtype & "', " &
                                        qty & ", " & cost & ", " & retail & ", '" & extractDate & "', '" & datetimenow & "', '" & item & "', '" &
                                        dim1 & "', '" & dim2 & "', '" & dim3 & "','" & lastUpdate & "','" & lastUser & "'"
                                        cmd = New SqlCommand(sql, con)
                                        cmd.CommandTimeout = 480
                                        cmd.ExecuteNonQuery()
                                    End If
    ...
    Private Sub Process_Other_Data(ByVal thePath, ByVal theTable, con, con2)
        ...
        Do
            Try
                ...
                Dim dt As DataTable = New DataTable
                Dim cnt As Int16 = 0
                Dim id, descr, dept, lastUser As String
                Dim eDate, lastUpdate As DateTime
                Dim xDate As Date
                Dim xmlFile As XmlReader
                            ...
                            If Not IsDBNull(oTest) And Not IsNothing(oTest) Then
                                ...
                                If theTable = "Classes" Then
                                    ...
                                    sql = "IF NOT EXISTS (SELECT ID FROM Classes WHERE ID = '" & id & "' AND Dept = '" & dept & "') " &
                                        "INSERT INTO Classes (ID, Dept, Description, Orig_Date, Status, Last_Change_Date, Last_Change_User) " &
                                        "SELECT '" & id & "','" & dept & "','" & descr & "','" & CDate(Date.Today) & "','Active', '" &
                                        lastUpdate & "','" & lastUser & " '" &
                                        "ELSE " &
                                        "UPDATE Classes SET Description = '" & descr & "', Dept = '" & dept & "', Extract_Date = '" & eDate & "', " &
                                        "Last_Change_Date = '" & lastUpdate & "', Last_Change_User = '" & lastUser & "' " &
                                ...
                                Else
                                    ...
                                    Else
                                        sql = "IF NOT EXISTS (SELECT ID FROM " & theTable & " WHERE ID = '" & id & "') " &
                                        "INSERT INTO " & theTable & " (ID, Description, Orig_Date, Status, Last_Change_Date, Last_Change_User) " &
                                        "SELECT '" & id & "','" & descr & "','" & CDate(Date.Today) & "','Active', '" & lastUpdate & "',' " &
                                        lastUser & "' " &
                                        "ELSE " &
                                        "UPDATE " & theTable & " SET Description = '" & descr & "', Extract_Date = '" & eDate & "', " &
                                        "Last_Change_Date = '" & lastUpdate & "', Last_Change_User = '" & lastUser & "' " &
               ...
               "INSERT INTO Buyers (ID, Description, Orig_Date, Last_Change_Date) " &
               "SELECT 'OTHER','MISCELLANEOUS BUYER','" & CDate(Date.Today) & "','" & Date.Now & "'"
                cmd = New SqlCommand(sql, con)
                cmd.CommandTimeout = 480
                cmd.ExecuteNonQuery()
                con.close()
